#include <iostream>
#include <bits/stdc++.h>

template <typename INPUT_T>
class Graf {
	enum Colors {
		WHITE,
	    GRAY,
	    BLACK,
	};

	struct Graf_elem {
		INPUT_T graf_elem_val;
		int graf_elem_ind;
		Colors color;
		Graf_elem() {}
		void operator()(INPUT_T ind) {
			graf_elem_ind = ind - 1;
			graf_elem_val = ind;
			color = WHITE;
		}
	};

	std::vector<std::vector<Graf_elem>> graf;
	size_t begin = 0;

	void Dfs(int ind, std::vector<INPUT_T>& res, bool& key) {
		if (graf[ind][0].color == BLACK) {
			return;
		}
		if (graf[ind][0].color == GRAY) {
			key = true;
			return;
		}
		graf[ind][0].color = GRAY;
		for (int i = 1; i < graf[ind].size(); ++i) {
			if (graf[ind][i].color == WHITE) {
				graf[ind][i].color = GRAY;
				Dfs(graf[ind][i].graf_elem_ind, res, key);
				graf[ind][i].color = BLACK;
			}
		}
		graf[ind][0].color = BLACK;
		res.push_back(graf[ind][0].graf_elem_val);
	}

public:

	void Build() {
		int n = graf.size();
		for(int i = 0; i < n; ++i) {
			if (graf[i].size() == 0) {
				Graf_elem elem;
				elem(i + 1);
				graf[i].push_back(elem);
			}
		}
	}

	Graf(size_t input_size) {
		graf.resize(input_size);
	}

	void Add(INPUT_T left_graf_val, INPUT_T right_graf_val) {
		Graf_elem left_graf_elem;
		left_graf_elem(left_graf_val);
		Graf_elem right_graf_elem;
		right_graf_elem(right_graf_val);
		graf[left_graf_elem.graf_elem_ind].push_back(right_graf_elem);
	}
	 
	void TopologicalSort() {
		std::vector<INPUT_T> res;
		bool key = false;
		int n = graf.size();
		for(int i = 0; i < n; ++i) {
			Dfs(i, res, key);
			if (key) {
				std::cout << -1 << std::endl;
				return;
			}
		}
		std::reverse(res.begin(), res.end());
		for(auto& element : res) {
	        std::cout << element << " ";
	    }
	    std::cout << std::endl;
	}
};

int main() {
	std::ios_base::sync_with_stdio(false);
	std::cin.tie(NULL);
	int n = 0, m = 0;
	int left = 0;
	int right = 0;
	int begin = 0;

	std::cin >> n >> m;

	Graf<int> graf(n);
	graf.Build();

	for(int i = 0; i < m; ++i) {
		std::cin >> left >> right;
		// right = i;
		graf.Add(left, right);
	}
	graf.TopologicalSort();
	return 0;
}
